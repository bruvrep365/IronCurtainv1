...
const COUNTRY_DEFS: { id: string; name: string; isoCode: string; alignment: Alignment; stability: number; military: number; economy: number; nukes: number; influence: { usa: number; ussr: number }; isContested: boolean; region: string; coastal: boolean; stateIds: string[] }[] = [
  ...
  { id: 'china', name: 'China', isoCode: '156', alignment: 'western', stability: 40, military: 60, economy: 25, nukes: 0, influence: { usa: 55, ussr: 15 }, isContested: true, region: 'ASIA', coastal: true, stateIds: ['cn_shanghai','cn_shanxi','cn_sichuan','cn_tibet','cn_yunnan','cn_guangdong'] },
...
function buildUSAFocus(): FocusTree {
  return {
    ...
    politicalPath: null,
    nodes: [
      { id: 'f1', name: 'Keynesian Stimulus', description: 'Massive domestic infrastructure spending. GDP +200, Production Points +30.', category: 'economic', status: 'available', turnsRequired: 12, turnsRemaining: 12, prerequisites: [], effects: { gdp: 200, researchPoints: 30 }, x: 0, y: 0 },
      { id: 'f2', name: 'Bretton Woods', description: 'Establish the global dollar system. Trade route income +50%.', category: 'economic', status: 'locked', turnsRequired: 24, turnsRemaining: 24, prerequisites: ['f1'], effects: { gdp: 300, prestige: 20 }, x: 0, y: 1 },
      { id: 'f3', name: 'Marshall Plan', description: 'Rebuild European allies. All NATO economies +15, Prestige +30.', category: 'economic', status: 'available', turnsRequired: 32, turnsRemaining: 32, prerequisites: ['f1'], effects: { gdp: 150, prestige: 30 }, x: 0, y: 2 },
      { id: 'f4', name: 'Dollar Hegemony', description: 'The dollar becomes the global reserve currency. Income +400/turn.', category: 'economic', status: 'locked', turnsRequired: 24, turnsRemaining: 24, prerequisites: ['f2', 'f3'], effects: { gdp: 400, prestige: 40 }, x: 0, y: 3 },
      { id: 'f5', name: 'Rearmament', description: 'Increase defense spending. Unlock Armor units. Military +10.', category: 'military', status: 'available', turnsRequired: 12, turnsRemaining: 12, prerequisites: [], effects: { unlockUnit: 'armor', military: 10 }, x: 1, y: 0 },
      { id: 'f6', name: 'NATO Formation', description: 'Formal military alliance with Western Europe. Allies gain +5 units.', category: 'military', status: 'locked', turnsRequired: 3, turnsRemaining: 3, prerequisites: ['f5'], effects: { prestige: 25, tension: 5 }, x: 1, y: 1 },
      { id: 'f7', name: 'Armored Divisions', description: 'Mass production of tanks. Armor cost -20%, unlock Air units.', category: 'military', status: 'locked', turnsRequired: 24, turnsRemaining: 24, prerequisites: ['f5'], effects: { unlockUnit: 'air', unitDiscount: 20 }, x: 1, y: 2 },
      { id: 'f8', name: 'Air Superiority', description: 'B-52 fleet and jet fighters. Air units cost -20%, +20 military power.', category: 'military', status: 'locked', turnsRequired: 32, turnsRemaining: 32, prerequisites: ['f6', 'f7'], effects: { military: 20, unitDiscount: 20 }, x: 1, y: 3 },
      { id: 'f9', name: 'Truman Doctrine', description: 'Containment policy formalized. Diplomacy power +20, Tension +10.', category: 'intelligence', status: 'available', turnsRequired: 1, turnsRemaining: 1, prerequisites: [], effects: { prestige: 10, tension: 10 }, x: 2, y: 0 },
      { id: 'f10', name: 'CIA Operations', description: 'Establish covert operations. Intelligence operations cost -1 AP.', category: 'intelligence', status: 'locked', turnsRequired: 12, turnsRemaining: 12, prerequisites: ['f9'], effects: { prestige: 15, researchPoints: 20 }, x: 2, y: 1 },
      { id: 'f11', name: 'Covert War', description: 'Sponsor proxy insurgencies. Proxy wars cheaper, tension lower.', category: 'intelligence', status: 'locked', turnsRequired: 32, turnsRemaining: 32, prerequisites: ['f9'], effects: { prestige: 20, tension: -10 }, x: 2, y: 2 },
      { id: 'f12', name: 'Space Race', description: 'NASA and Apollo program. Prestige +50, unlocks Satellite units.', category: 'intelligence', status: 'locked', turnsRequired: 48, turnsRemaining: 48, prerequisites: ['f10', 'f11'], effects: { prestige: 50, tension: -15, gdp: 200 }, x: 2, y: 3 },
    ]
  };
}
...
function buildUSSRFocus(): FocusTree {
  return {
    ...
    nodes: [
      // === ECONOMIC ===
      { id: 's1', name: 'Five-Year Plan', description: 'Centralized industrial expansion. GDP +150, Production +25.', category: 'economic', status: 'available', turnsRequired: 12, turnsRemaining: 12, prerequisites: [], effects: { gdp: 150, researchPoints: 25 }, x: 0, y: 0 },
      { id: 's2', name: 'COMECON', description: 'Economic integration of Eastern Bloc. All Warsaw economies +10.', category: 'economic', status: 'locked', turnsRequired: 24, turnsRemaining: 24, prerequisites: ['s1'], effects: { gdp: 200, prestige: 15 }, x: 0, y: 1 },
      { id: 's3', name: 'Soviet Oil Exports', description: 'Fuel the global economy. Income +250, but Tension +10.', category: 'economic', status: 'locked', turnsRequired: 12, turnsRemaining: 12, prerequisites: ['s1'], effects: { gdp: 250, tension: 10 }, x: 0, y: 2 },
      { id: 's4', name: 'Moscow Financial Center', description: 'The Ruble challenges the dollar. Income +350, Prestige +35.', category: 'economic', status: 'locked', turnsRequired: 100, turnsRemaining: 100, prerequisites: ['s2', 's3'], effects: { gdp: 350, prestige: 35 }, x: 0, y: 3 },
      // === MILITARY ===
      { id: 's5', name: 'Red Army Expansion', description: 'Massive conscription. Military +15, unlock Armor.', category: 'military', status: 'available', turnsRequired: 12, turnsRemaining: 12, prerequisites: [], effects: { unlockUnit: 'armor', military: 15 }, x: 1, y: 0 },
      { id: 's6', name: 'Warsaw Pact', description: 'Military alliance with Eastern Bloc. Allies gain +5 units.', category: 'military', status: 'locked', turnsRequired: 12, turnsRemaining: 12, prerequisites: ['s5'], effects: { prestige: 25, tension: 5 }, x: 1, y: 1 },
      { id: 's7', name: '"три танкиста"', description: 'T-54 mass production. Armor cost -20%, unlock Air.', category: 'military', status: 'locked', turnsRequired: 3, turnsRemaining: 3, prerequisites: ['s5'], effects: { unlockUnit: 'air', unitDiscount: 20 }, x: 1, y: 2 },
      { id: 's8', name: 'Send Advisors to Vietnam', description: 'Guerrilla warfare expertise. +20 military, proxy wars cheaper.', category: 'military', status: 'locked', turnsRequired: 4, turnsRemaining: 4, prerequisites: ['s6', 's7'], effects: { military: 20, unitDiscount: 20 }, x: 1, y: 3 },
      // === INTELLIGENCE ===
      { id: 's9', name: 'Establishment of the KGB', description: 'Establish intelligence network. Tension -10, Prestige +15.', category: 'intelligence', status: 'available', turnsRequired: 1, turnsRemaining: 1, prerequisites: [], effects: { prestige: 15, tension: -10 }, x: 2, y: 0 },
      { id: 's10', name: 'Measures of National Importance', description: 'Soviet disinformation campaigns. Intelligence ops cost -1 AP.', category: 'intelligence', status: 'locked', turnsRequired: 3, turnsRemaining: 3, prerequisites: ['s9'], effects: { prestige: 15, researchPoints: 20 }, x: 2, y: 1 },
      { id: 's11', name: 'Sputnik', description: 'First satellite launch. Prestige +40, Tension -10.', category: 'intelligence', status: 'locked', turnsRequired: 3, turnsRemaining: 3, prerequisites: ['s9'], effects: { prestige: 40, tension: -10 }, x: 2, y: 2 },
      { id: 's12', name: 'Lunar Program', description: 'Soviet space supremacy. Prestige +50, +200 GDP.', category: 'intelligence', status: 'locked', turnsRequired: 4, turnsRemaining: 4, prerequisites: ['s10', 's11'], effects: { prestige: 50, gdp: 200 }, x: 2, y: 3 },
      // === POLITICAL — SHARED PRE-CHOICE SEQUENCE ===
      { id: 'sp0', name: 'Postwar Reorganisation', description: 'The Great Patriotic War is over. The Soviet Union must rebuild its shattered state and define its postwar identity. GDP +50, Prestige +10.', category: 'political', status: 'available', turnsRequired: 4, turnsRemaining: 4, prerequisites: [], effects: { gdp: 50, prestige: 10 }, x: 1, y: 0 },
      { id: 'sp1', name: 'Soviet Sphere of Influence', description: 'Consolidate control over Eastern Europe. Install friendly governments in Poland, Hungary, Romania, and Bulgaria. Prestige +15, Tension +5.', category: 'political', status: 'locked', turnsRequired: 6, turnsRemaining: 6, prerequisites: ['sp0'], effects: { prestige: 15, tension: 5 }, x: 1, y: 1 },
      { id: 'sp2', name: 'Cominform', description: 'Establish the Communist Information Bureau to coordinate international communist parties. The bipolar world crystallises. Prestige +10, Research +10, Tension +3.', category: 'political', status: 'locked', turnsRequired: 3, turnsRemaining: 3, prerequisites: ['sp1'], effects: { prestige: 10, researchPoints: 10, tension: 3 }, x: 1, y: 2 },
      { id: 'sp3', name: 'Berlin Blockade', description: 'Seal off West Berlin to force the Western Allies out. A bold gambit that shakes the world. Military +5, Prestige +15, Tension +10.', category: 'political', status: 'locked', turnsRequired: 4, turnsRemaining: 4, prerequisites: ['sp2'], effects: { military: 5, prestige: 15, tension: 10 }, x: 1, y: 3 },
      { id: 'sp4', name: 'Ideological Direction', description: 'The Soviet Union must define its political character. Choose the Stalinist path of iron discipline or the Reformist path of controlled liberalisation. This choice is permanent.', category: 'political', status: 'locked', turnsRequired: 1, turnsRemaining: 1, prerequisites: ['sp3'], effects: {}, x: 1, y: 4 },
      // === POLITICAL — STALINIST PATH ===
      // Historical branch
      { id: 'sp_s1', name: 'Cult of Personality', description: 'Elevate Stalin to divine status. Prestige +30, Tension +8. The party is purged of moderates.', category: 'political', status: 'locked', turnsRequired: 8, turnsRemaining: 8, prerequisites: ['sp4'], effects: { prestige: 30, tension: 8 }, x: 0, y: 5 },
      { id: 'sp_s2', name: 'Stalinist Collectivization', description: 'Forced collectivization of agriculture. GDP +100, Military +5, Tension +8.', category: 'political', status: 'locked', turnsRequired: 12, turnsRemaining: 12, prerequisites: ['sp_s1'], effects: { gdp: 100, military: 5, tension: 8 }, x: 0, y: 6 },
      { id: 'sp_s3', name: 'Gulag Labour Economy', description: 'Forced labour drives industrial output. GDP +200, Military +10, Tension +12.', category: 'political', status: 'locked', turnsRequired: 6, turnsRemaining: 6, prerequisites: ['sp_s2'], effects: { gdp: 200, military: 10, tension: 12 }, x: 0, y: 7 },
      { id: 'sp_s4', name: 'Great Purge', description: 'Purge unreliable officers and party members. Military +20, Prestige +15, Tension +10.', category: 'political', status: 'locked', turnsRequired: 3, turnsRemaining: 3, prerequisites: ['sp_s2'], effects: { military: 20, prestige: 15, tension: 10 }, x: 1, y: 7 },
      { id: 'sp_s5', name: 'Doctors\' Plot', description: 'Show-trials crush internal opposition. Prestige +20, Tension +5. Unlocks non-historical branch.', category: 'political', status: 'locked', turnsRequired: 2, turnsRemaining: 2, prerequisites: ['sp_s3', 'sp_s4'], effects: { prestige: 20, tension: 5 }, x: 0, y: 8 },
      { id: 'sp_s6', name: 'Consolidation of Marxist-Leninist Thought', description: 'Crush dissent across Eastern Europe. Military +15, Tension +15, Prestige +10.', category: 'political', status: 'locked', turnsRequired: 3, turnsRemaining: 3, prerequisites: ['sp_s4'], effects: { military: 15, tension: 15, prestige: 10 }, x: 1, y: 8 },
      // Non-historical escalation
      { id: 'sp_s7', name: 'Stalin Lives On', description: 'Stalin lives! Prestige +50, Military +20, Tension +20.', category: 'political', status: 'locked', turnsRequired: 1, turnsRemaining: 1, prerequisites: ['sp_s5'], effects: { prestige: 50, military: 20, tension: 20 }, x: 0, y: 9 },
      { id: 'sp_s8', name: 'Assured Retaliation', description: 'Develop first-strike nuclear doctrine. Nukes +3, Military +20, Tension +20.', category: 'political', status: 'locked', turnsRequired: 6, turnsRemaining: 6, prerequisites: ['sp_s6', 'sp_s7'], effects: { nuclearWarheads: 3, military: 20, tension: 20 }, x: 1, y: 9 },
      { id: 'sp_s9', name: 'Worldwide Revolution!', description: 'Proclaim inevitable global communist triumph. Prestige +80, GDP +300, Tension +25.', category: 'political', status: 'locked', turnsRequired: 12, turnsRemaining: 12, prerequisites: ['sp_s7'], effects: { prestige: 80, gdp: 300, tension: 25 }, x: 0, y: 10 },
      { id: 'sp_s10', name: 'Seven Days to the River Rhine', description: 'Total mobilisation for preemptive strike. Military +40, Nukes +2, Tension +30.', category: 'political', status: 'locked', turnsRequired: 24, turnsRemaining: 24, prerequisites: ['sp_s8', 'sp_s9'], effects: { military: 40, nuclearWarheads: 2, tension: 30 }, x: 0, y: 11 },
      ...
      { id: 'sp_r1', name: 'De-Stalinization', description: 'REFORMIST: Khrushchev\'s secret speech denounces Stalin\'s crimes. Prestige +20, Tension -15. The thaw begins.', category: 'political', status: 'locked', turnsRequired: 24, turnsRemaining: 24, prerequisites: ['sp4'], effects: { prestige: 20, tension: -15 }, x: 2, y: 5 },
      { id: 'sp_r2', name: 'New Economic Policy', description: 'REFORMIST: Liberalise agriculture and small enterprise. GDP +180, Prestige +15, Tension -5.', category: 'political', status: 'locked', turnsRequired: 24, turnsRemaining: 24, prerequisites: ['sp_r1'], effects: { gdp: 180, prestige: 15, tension: -5 }, x: 2, y: 6 },
      { id: 'sp_r3', name: 'Cultural Thaw', description: 'REFORMIST: Loosen artistic censorship — Pasternak, Solzhenitsyn. Prestige +25, Research +20, Tension -8.', category: 'political', status: 'locked', turnsRequired: 12, turnsRemaining: 12, prerequisites: ['sp_r1'], effects: { prestige: 25, researchPoints: 20, tension: -8 }, x: 3, y: 6 },
      { id: 'sp_r4', name: 'Helsinki Accords', description: 'REFORMIST: Sign human rights and security agreements with the West. Prestige +35, Tension -20. Opens diplomatic channels.', category: 'political', status: 'locked', turnsRequired: 3, turnsRemaining: 3, prerequisites: ['sp_r2', 'sp_r3'], effects: { prestige: 35, tension: -20 }, x: 2, y: 7 },
      { id: 'sp_r5', name: 'Glasnost', description: 'REFORMIST: Policy of openness and transparency. Prestige +30, Research +30, GDP +100, Tension -10.', category: 'political', status: 'locked', turnsRequired: 48, turnsRemaining: 48, prerequisites: ['sp_r4'], effects: { prestige: 30, researchPoints: 30, gdp: 100, tension: -10 }, x: 2, y: 8 },
      { id: 'sp_r6', name: 'Perestroika', description: 'REFORMIST: Restructure the Soviet economy. GDP +250, Prestige +25, Research +40, Tension -10.', category: 'political', status: 'locked', turnsRequired: 52, turnsRemaining: 52, prerequisites: ['sp_r5'], effects: { gdp: 250, prestige: 25, researchPoints: 40, tension: -10 }, x: 3, y: 8 },
      { id: 'sp_r7', name: 'Arms Reduction Treaties', description: 'REFORMIST: Negotiate SALT and START with the USA. Tension -25, Prestige +40. Nuclear arsenals draw down.', category: 'political', status: 'locked', turnsRequired: 4, turnsRemaining: 4, prerequisites: ['sp_r5'], effects: { prestige: 40, tension: -25 }, x: 2, y: 9 },
      { id: 'sp_r8', name: 'Soviet Commonwealth', description: 'REFORMIST: Voluntary federation of Soviet republics — a transformed, stable USSR. GDP +300, Prestige +50, Tension -20.', category: 'political', status: 'locked', turnsRequired: 12, turnsRemaining: 12, prerequisites: ['sp_r6', 'sp_r7'], effects: { gdp: 300, prestige: 50, tension: -20 }, x: 2, y: 10 },
      { id: 'sp_r9', name: 'NEW SOVIET CENTURY', description: 'REFORMIST VICTORY: A reformed, stable Soviet state leads the world through diplomacy and economic might. Prestige +120, GDP +400, Tension -30. Completing this wins the game for the USSR by soft power.', category: 'political', status: 'locked', turnsRequired: 16, turnsRemaining: 16, prerequisites: ['sp_r8'], effects: { prestige: 120, gdp: 400, tension: -30 }, x: 2, y: 11 },
    ]
  };
}
